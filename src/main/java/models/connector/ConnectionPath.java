...
public class ConnectionPath implements IObserver, IConnectorsPiece {
    ...
    private ObservableList<Line> pathLinesList = FXCollections.observableArrayList();
    ...
    void buildPath() {
        if (mediator.getStartSocket().getRole() == Socket.Role.Output) {
            startPathPoint = mediator.getStartSocket().getCoveredPathPoint();
            endPathPoint = mediator.getEndSocket().getCoveredPathPoint();
        } else if (mediator.getStartSocket().getRole() == Socket.Role.Input) {
            startPathPoint = mediator.getEndSocket().getCoveredPathPoint();
            endPathPoint = mediator.getStartSocket().getCoveredPathPoint();
        }
...
//    }
    ...
    private void generatePath(PathPoint startPathPoint) {
        System.out.println("STARTPOINT " + startPathPoint);
    ...
    private void getAdjacentPoints(PathPoint currentPoint) {
        ...
        if (leftC >= 0) {
            checkThisPoint(currentPoint, pathPoints[leftC][currRow]);//Проверка левого соседа;
            pathPoints[leftC][currRow].setRelativeDir(RelativeDirectionStatus.Left);
            pathPoints[leftC][currRow].setPriority();
        }
        if (rightC < pathPoints[0].length) {
            checkThisPoint(currentPoint, pathPoints[rightC][currRow]);//Проверка правого соседа;
            pathPoints[rightC][currRow].setRelativeDir(RelativeDirectionStatus.Right);
            pathPoints[rightC][currRow].setPriority();
        }
        if (downR < pathPoints.length) {
            checkThisPoint(currentPoint, pathPoints[currCol][downR]);//Проверка нижнего соседа;
            pathPoints[currCol][downR].setRelativeDir(RelativeDirectionStatus.Down);
            pathPoints[currCol][downR].setPriority();
        }
        if (upR >= 0) {
            checkThisPoint(currentPoint, pathPoints[currCol][upR]);//Проверка верхнего соседа;
            pathPoints[currCol][upR].setRelativeDir(RelativeDirectionStatus.Up);
            pathPoints[currCol][upR].setPriority();
        }
    }
